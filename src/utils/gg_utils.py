...
def download_media(url: str, directory: str, name: str, media_type: str = "img") -> int:
    """
    Parameters:
    ...
    while num_of_tried < 2:
        ...
        try:
            ...
            if isinstance(url, str):
                # url = quote(url, safe = ':/')
                logger.info(
                    f"Downloading: url : {url}, directory : {directory}, name : {name}"
                )
                if "&export=download" in url or media_type == "mp4":
                    ...
                    if response.status_code == 200:
                        ...
                        logger.error(
                            f"Failed to download image: {url} -- Status code: {response.status_code}"
                        )
                        return -1
                else:
        ...
        ) as e:  # exception when image is broken, try send request again
            logger.error(
                f"Error {e} -- Retry Download img = {name}; path = {directory}"
            )
            num_of_tried += 1
...
def check_last_value_in_column(
    spreadsheetId, sheet_name: str, column_search: str, start_row: int = 1
) -> Tuple[int, Any]:
    try:
        ...
        _result = (
            sheet.values().get(spreadsheetId=spreadsheetId, range=_range).execute()
        )
...
def check_last_value_in_row(
    spreadsheetId: str, sheet, sheet_name: str, row_search: int, start_column: str = "A"
) -> Tuple[str, Any]:
    try:
        ...
        for i in reversed(range(len(row_values))):
            if (
                row_values[i] and row_values[i][0].strip()
            ):  # if not empty or just spaces
...
def check_for_checked_rows(
    spreadsheetId: str, sheet, sheet_name: str, column_search: str, start_row: int = 2
) -> list:
    try:
        ...
        for i in range(len(_values)):
            if (
                _values[i] and _values[i][0].strip() == "TRUE"
            ):  # if not empty or just spaces
                checked_rows.append(
                    i + start_row
                )  # Because we start from row i in the sheet
        return checked_rows
    except Exception as e:
        logger.error(f"Error {e}")
        return -1
    ...
    try:
        ...
        if os.path.exists("ThisNotSecretKeyAtAll/token.json"):
            creds = Credentials.from_authorized_user_file(
                "ThisNotSecretKeyAtAll/token.json", SCOPES
            )
...
def create_new_sheets_with_template(
    ...
    try:
        ...
        src_range = (
            f"{source_sheet_name}!{start_column}{row_number}:{last_column}{row_number}"
        )
        # Get the values from the source sheet
        copy_src_range = (
            sheet.values().get(spreadsheetId=spreadsheetId, range=src_range).execute()
        )
        ...
        if not is_sh_exist:
            ...
            add_sheet_request = {
                "addSheet": {"properties": {"title": target_sheet_name}}
            }
            sheet.batchUpdate(
                spreadsheetId=spreadsheetId, body={"requests": [add_sheet_request]}
            ).execute()
        else:
            clear_range = (
                f"{target_sheet_name}!A2:ZZ1000000"  # Use a large number for rows
            )
            sheet.values().clear(
                spreadsheetId=spreadsheetId, range=clear_range
            ).execute()
        dest_range = (
            f"{target_sheet_name}!{start_column}{row_number}:{last_column}{row_number}"
        )
        update_body = {
        ...
        logger.info(
            f"Row {row_number} copied from {source_sheet_name} to {target_sheet_name}."
        )
    except Exception as e:
        logger.error(f"Error: {e}")
...
def write_to_gsheet(
    data: Dict[str, List[str]], spreadsheetId: str, sheet_name: str
) -> None:
    try:
        ...
        update_date = [
            [str(datetime.now())] for _ in range(value, value + len(data["title"]))
        ]  # Update date
...
def get_value_from_row(gsheet_read, range_name, spreadsheetId) -> Union[str, None]:
    try:
        result_ = gsheet_read.read_from_gsheet(
            range_name=range_name, spreadsheetId=spreadsheetId
        )
