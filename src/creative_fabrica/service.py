...
from src.creative_fabrica.config import BASE_URL, UPLOAD_SUFFIX, SUCCESS_URL
...
class UploadFile:
    ...
    def get_sites(self) -> int:
        ...
        WebDriverWait(self.driver, 50).until(
            EC.presence_of_element_located((By.XPATH, CreateFabricaElems.PAGE_TITLE))
        )
        return 1
    ...
    def write_category(self) -> int:
        ...
        write_with_delay(
            element=category_input, message=self.current_item.category, interval=0.01
        )
    ...
    def write_price(self) -> int:
        ...
        price = WebDriverWait(self.driver, 30).until(
            EC.presence_of_element_located((By.XPATH, CreateFabricaElems.PRICE))
        )
    ...
    def write_description(self) -> int:
        ...
        description = WebDriverWait(self.driver, 30).until(
            EC.presence_of_element_located((By.XPATH, CreateFabricaElems.DESCRIPTION))
        )
    ...
    def write_tags(self) -> int:
        ...
        tags = WebDriverWait(self.driver, 10).until(
            EC.presence_of_element_located((By.XPATH, CreateFabricaElems.TAGS))
        )
    ...
    def check_upload_status(self, timeout: int = 300) -> int:
        ...
        expected_upload = (
            len(self.current_item.img_files) + 1
        )  # Must upload all items in img_files and a zip file
