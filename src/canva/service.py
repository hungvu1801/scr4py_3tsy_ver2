...
from src.utils.action_utils import scroll_to_elem, scroll_page_down
    ...
    def find_img_elem(self, filter_edit: bool = True) -> int:
        ...
        img_lst = WebDriverWait(self.driver, 50).until(
            EC.presence_of_all_elements_located(
                (By.XPATH, CanvaElems.IMG_ELEM)))
        has_img = 0
        for img in img_lst:
            img_name = img.find_element(By.XPATH, CanvaElems.IMG_NAME).text
            if filter_edit:
                if img_name not in self.processed_img and "edit" not in img_name.lower():
                    has_img = 1
                    self.img = img
